...
def home(request):
    blog = {}
    max_tag = Tag.objects.filter(blog__nums__view__gt=1000)
    max_view = Blog.objects.filter(nums__view__gt=1000, published=True)
    form = SearchForm()
    try:
    ...
    if request.session['blogsearch']:
        blog = Blog.objects.annotate(blogsearch=TrigramSimilarity('titel', request.session.get('search')),).filter(
...
def detail(request, id):
    ...
    prodacts = Prodact.objects.filter(tags__in=tag, published=True)
        ...
        if request.method == "POST":
            ...
            if form.is_valid():
                ...
                blog = Blog.objects.create(
                        ...
                        image=cd['image'], music=cd['music'],
                        film=cd['film'], author=user)
